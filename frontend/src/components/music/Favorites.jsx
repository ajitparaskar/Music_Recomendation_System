...
const FavoritesSkeleton = () => (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-32">
    {Array.from({ length: 6 }).map((_, index) => (
      <div key={index} className="glass-surface rounded-3xl p-6 space-y-4">
        <div className="skeleton h-6 rounded-xl w-2/3"></div>
        <div className="skeleton h-4 rounded-lg w-1/2"></div>
        <div className="skeleton h-10 rounded-2xl w-full"></div>
      </div>
    ))}
  </div>
);
  ...
  if (!token) {
    return (
      <div className="min-h-screen py-32 px-6 flex flex-col items-center justify-center">
        ...
        <p className="app-muted">You need to be logged in to view your favorites.</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen py-24 px-6 relative z-10 max-w-7xl mx-auto">
      <div className="flex items-center gap-3 mb-10">
        ...
        <h2 className="text-4xl font-extrabold app-text">{user?.username}'s Library</h2>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 glass-surface rounded-3xl">
          ...
          <h3 className="text-2xl font-bold app-text mb-2">No favorites yet</h3>
          <p className="app-muted">Discover new music on the home page and save it here.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-32">
          {favorites.map((song, index) => (
            <motion.div
              ...
              className="group glass-surface p-6 rounded-3xl hover:border-primary/50 hover:bg-white/10 transition-all duration-300 cursor-pointer flex flex-col items-start gap-4"
            >
              <div className="w-full flex justify-between items-start">
                <div className="flex-1 min-w-0 pr-4">
                  <h3 className="text-xl font-bold app-text truncate mb-1 bg-clip-text group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-primary group-hover:to-secondary transition-all">
                    {song.song_title}
                  </h3>
                  <p className="app-muted text-sm truncate">{song.artist}</p>
                </div>
                <button
                  onClick={(event) => removeFavorite(event, song.id)}
              ...
              <div className="w-full mt-auto pt-4 border-t border-white/10 flex items-center justify-between text-sm group-hover:border-primary/30 transition-colors">
                {song.youtube_video_id ? (
                  ...
                  <span className="app-muted">Audio only</span>
                )}
              </div>
            </motion.div>
          ))}
